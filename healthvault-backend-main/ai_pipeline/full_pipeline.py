...
def process_medical_document(image_path, patient_id="P-94821", visit_id="VIS-1001", doctor_name="Dr. Arthur Vance, M.D."):
    ...
    print(f"[OK] OCR complete! Engine: {ocr_result['engine']}, Confidence: {ocr_result['avg_confidence']}%, Length: {len(ocr_result['full_text'])} chars")
    print(f"[OK] Cleaned Debug Image Saved: {ocr_result['cleaned_image_path']}")

    # Memory cleanup after OCR
    import gc
    gc.collect()
    print("[OK] Memory cleanup complete after OCR stage.")
    ...
    try:
        ...
        rss4 = _get_process_rss_mb()
        print(f"[MEM_DIAGNOSTIC] [Stage 4] RSS after medical NER/entity extraction: {rss4:.1f} MB")
    except Exception:
        pass

    # Fully release EasyOCR Reader and PyTorch weights before subsequent stages
    try:
        from ocr_pipeline import cleanup_ocr_reader
        cleanup_ocr_reader()
    except Exception as e:
        print(f"[full_pipeline] Warning during OCR cleanup: {e}")

    print(f"[OK] Medical NER complete! Meds: {len(ner_result.get('medications', []))}, Diagnoses: {len(ner_result.get('diagnoses', []))}, Labs: {len(ner_result.get('lab_results', []))}")
