...
def initialize_ai_pipeline(vector_store_location: str = ":memory:") -> Dict[str, Any]:
    """
    Initializes BGE-M3, Qdrant, Cross-Encoder, and LangGraph once globally.
    Models do NOT reload on subsequent API requests.
    """
    global _PIPELINE_INSTANCES
    if "agents" in _PIPELINE_INSTANCES:
        return _PIPELINE_INSTANCES

    print("[pipeline_api] Initializing HealthVault AI Person 3 Pipeline...")
    ...
    print("[pipeline_api] Pipeline initialized successfully!")
    return _PIPELINE_INSTANCES
...
def index_patient_records(records: List[Dict[str, Any]]) -> int:
    ...
    # Invalidate per-patient BM25 index cache for newly updated patients
